      ...
      <div className="flex-1 h-5 bg-slate-700/30 rounded-full overflow-hidden relative">
        ...
        >
          <motion.div
            ...
            transition={{ delay: delay + 0.3, duration: 1.2, repeat: 0 }}
...
export default function StatCompareCard({ payments = [], expenses = [], leads = [], clients = [] }) {
  const now = new Date();
  const thisMonthStart = new Date(now.getFullYear(), now.getMonth(), 1);
  const lmStart = new Date(now.getFullYear(), now.getMonth() - 1, 1);
  const lmEnd = new Date(now.getFullYear(), now.getMonth(), 0, 23, 59, 59);

  const inMTD = (d) => d && new Date(d) >= thisMonthStart && new Date(d) <= now;
  ...
  const netChange = lmIncome - lmExpenses !== 0
    ? Math.round(((netMTD - netLM) / Math.abs(netLM)) * 100)
    : netMTD > 0 ? 100 : 0;
  ...
  return (
    <motion.div
      ...
      className="bg-slate-800/50 rounded-lg border border-slate-700/50 p-5 relative overflow-hidden"
    >
      {/* Ambient glow */}
      <motion.div
        initial={{ opacity: 0 }}
        ...
        style={{ background: netMTD >= 0 ? '#34d399' : '#f87171' }}
      />
      ...
      <div className="flex items-center justify-between mb-4 relative z-10">
        ...
        <motion.span
          ...
          className="text-[10px] font-medium text-slate-500 bg-slate-700/30 px-2 py-0.5 rounded-full"
        >
          MTD
        </motion.span>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4 relative z-10">
        {[
          ...
          <motion.div
            ...
            className={`text-center p-2.5 rounded-lg bg-slate-900/40 border ${item.borderColor}`}
          >
            ...
            <p className={`text-lg font-bold ${item.color} tabular-nums`}>
              ${Math.round(typeof item.val === 'number' ? item.val : 0).toLocaleString()}
            </p>
            {item.label === 'Net' && (
              <motion.div
                ...
                className={`inline-flex items-center gap-0.5 mt-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                  netChange >= 0 ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
                }`}
              >
      ...
      <motion.div
        ...
        className="h-28 mb-4 relative z-10"
      >
      ...
      {/* Last month comparison footer */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 1, duration: 0.5 }}
        className="mt-4 pt-3 border-t border-slate-700/40 flex items-center justify-between relative z-10"
      >
        <div className="flex items-center gap-3">
          <div className="text-center">
            <p className="text-[10px] text-slate-500">LM Income</p>
            <p className="text-xs font-bold text-slate-400">${lmIncome.toLocaleString()}</p>
          </div>
          <ArrowRight className="w-3 h-3 text-slate-600" />
          <div className="text-center">
            <p className="text-[10px] text-slate-500">MTD Income</p>
            <p className="text-xs font-bold text-emerald-400">${mtdIncome.toLocaleString()}</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-center">
            <p className="text-[10px] text-slate-500">LM Expenses</p>
            <p className="text-xs font-bold text-slate-400">${lmExpenses.toLocaleString()}</p>
          </div>
          <ArrowRight className="w-3 h-3 text-slate-600" />
          <div className="text-center">
            <p className="text-[10px] text-slate-500">MTD Expenses</p>
            <p className="text-xs font-bold text-red-400">${mtdExpenses.toLocaleString()}</p>
          </div>
        </div>
      </motion.div>
    </motion.div>
