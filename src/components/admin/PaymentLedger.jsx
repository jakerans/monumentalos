...
const METHOD_LABELS = { ach: 'ACH', check: 'Check', wire: 'Wire', credit_card: 'Card', cash: 'Cash', other: 'Other', invoice: 'Invoice' };
        ...
        {filtered.length === 0 ? (
          ...
          <FlipMove duration={350} easing="cubic-bezier(0.25, 0.46, 0.45, 0.94)" staggerDurationBy={20} enterAnimation="fade" leaveAnimation="fade">
            {filtered.sort((a, b) => new Date(b.date) - new Date(a.date)).map(p => (
              <PaymentRow key={p.id} p={p} getClientName={getClientName} METHOD_LABELS={METHOD_LABELS} />
            ))}
          </FlipMove>
        )}
      </ScrollArea>
...
const PaymentRow = forwardRef(({ p, getClientName, METHOD_LABELS }, ref) => (
  <div ref={ref} className="px-4 py-2 flex items-center justify-between hover:bg-slate-700/20 border-b border-slate-700/30 last:border-b-0">
    <div className="min-w-0">
      <div className="flex items-center gap-1.5">
        ...
        {p.invoice_id && <span className="text-[9px] px-1 py-0.5 rounded bg-blue-500/15 text-blue-400 font-medium">{p.invoice_id}</span>}
      </div>
    ...
    <span className="text-sm font-bold text-emerald-600 flex-shrink-0">+${p.amount?.toLocaleString()}</span>
  </div>
));