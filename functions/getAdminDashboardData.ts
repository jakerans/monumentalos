...
Deno.serve(async (req) => {
  try {
    ...
    activeClients.forEach(client => {
      ...
      if (bt === 'pay_per_show') {
        cLeads.filter(l => l.disposition === 'showed' && inMTD(l.appointment_date)).forEach(l => { liveAccruals += getLeadPrice(client, l, 'price_per_show'); });
    ...
    activeEmployees.forEach(emp => {
      ...
      if (emp.classification === 'salary' && emp.pay_per_cycle) {
        ...
        if (emp.contractor_billing_type === 'monthly' && emp.contractor_rate) {
          monthlyCost = emp.contractor_rate;
        } else if (emp.contractor_billing_type === 'per_cycle' && emp.pay_per_cycle) {
          monthlyCost = (emp.pay_per_cycle * cyclesPerYear) / 12;
        } else if (emp.contractor_billing_type === 'hourly' && emp.contractor_rate) {
          monthlyCost = emp.contractor_rate * (emp.standard_monthly_hours || 160);
        }
      }
    ...
    const recordedExpensesMTD = expenses.filter(e => inMTD(e.date) && e.expense_type !== 'distribution').reduce((s, e) => s + effAmt(e), 0);
    const recordedCogsMTD = expenses.filter(e => inMTD(e.date) && e.expense_type === 'cogs').reduce((s, e) => s + effAmt(e), 0);
    ...
    const calcPeriod = (rangeFn) => {
      ...
      const rangeExpenses = expenses.filter(e => rangeFn(e.date) && e.expense_type !== 'distribution' && e.category !== 'distribution');
    ...
    const mtdExpenses = expenses.filter(e => inMTD(e.date) && e.expense_type !== 'distribution' && e.category !== 'distribution').reduce((s, e) => s + effAmt(e), 0);
    const lmIncome = paidBillingRecords.filter(b => inLM(b.paid_date)).reduce((s, b) => s + (b.paid_amount || b.calculated_amount || 0), 0);
    const lmExpenses = expenses.filter(e => inLM(e.date) && e.expense_type !== 'distribution' && e.category !== 'distribution').reduce((s, e) => s + effAmt(e), 0);
    const cogsTotal = expenses.filter(e => inMTD(e.date) && e.expense_type === 'cogs').reduce((s, e) => s + effAmt(e), 0);
    ...
    const chartData14 = (() => {
      ...
      for (let i = 13; i >= 0; i--) {
        ...
        const dayExpense = expenses.filter(e => (e.date || '').startsWith(dayStr) && e.expense_type !== 'distribution' && e.category !== 'distribution').reduce((s, e) => s + effAmt(e), 0);
