...
export default function ClientGoalEditor({ client, onSaved }) {
  ...
  return (
    <div>
      ...
      <div className="space-y-2 bg-slate-900/50 border border-slate-700/50 rounded-lg p-2.5">
        <div className="grid grid-cols-2 gap-1.5">
          <div>
            <label className="block text-[9px] font-medium text-slate-400 mb-0.5">Goal Type</label>
            <select
              value={goalType}
              onChange={(e) => setGoalType(e.target.value)}
              className="w-full px-2 py-1.5 text-xs border border-slate-700 rounded-md bg-slate-800 text-white focus:outline-none focus:ring-2 focus:ring-[#D6FF03]"
            >
          ...
          <div>
            <label className="block text-[9px] font-medium text-slate-400 mb-0.5">Target #</label>
            <input
              type="number"
              value={goalValue}
              ...
              placeholder="e.g. 20"
              disabled={!goalType}
              className="w-full px-2 py-1.5 text-xs border border-slate-700 rounded-md bg-slate-800 text-white focus:outline-none focus:ring-2 focus:ring-[#D6FF03] disabled:opacity-50"
            />
          </div>
        </div>
        {goalType && goalValue && (
          <div>
            <label className="block text-[9px] font-medium text-slate-400 mb-1">Status</label>
