...
import { X, Phone, Mail, Building2, Calendar, Clock, FileText, Briefcase, Ruler, DollarSign } from 'lucide-react';
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export default function LeadDetailPanel({ lead, clientName, open, onOpenChange, onAction }) {
  if (!lead) return null;
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="flex gap-2">
            {lead.status === 'new' && (
              ...
              >
                <Phone className="w-3.5 h-3.5 inline mr-1.5" />Mark First Call
              </button>
            )}
            {(lead.status === 'first_call_made' || lead.status === 'contacted') && !lead.appointment_date && (
              ...
              >
                <Calendar className="w-3.5 h-3.5 inline mr-1.5" />Book Appointment
              </button>
            )}
          </div>
          ...
          <div>
            ...
            <div className="space-y-2.5">
              ...
              {lead.speed_to_lead_minutes != null && (
                <InfoRow icon={Clock} label="Speed to Lead" value={`${lead.speed_to_lead_minutes} minutes`} />
              )}
            </div>
          </div>
