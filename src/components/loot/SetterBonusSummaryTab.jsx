...
export default function SetterBonusSummaryTab({ defaultPeriod }) {
  ...
  const handleMarkLootPaid = async (setter) => {
    ...
    toast({ title: 'Loot box prizes approved for payroll' });
        ...
        <div key={setter.setter_id} className="bg-slate-800/60 rounded-xl border border-slate-700/50 p-4 mb-3">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-[11px] px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300">
              📦 Loot Boxes: {setter.loot_total > 0 ? `$${setter.loot_total}` : 'None'}
            </span>
          </div>
          ...
          {expanded[setter.setter_id] && (
            <div className="mt-3 space-y-3">
              ...
              {setter.loot_items.length > 0 && (
                <div>
                  <p className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Loot Boxes</p>
                  <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500 italic">Spiff totals and approved loot box prizes are automatically included when you run payroll.</p>
            </div>
          )}
        </div>
      ))}
